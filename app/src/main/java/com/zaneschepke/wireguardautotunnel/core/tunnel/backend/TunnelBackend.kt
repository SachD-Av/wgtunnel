...
interface TunnelBackend {
    ...
    /**
     * Forces the tunnel's UDP socket to rebind to the current network.
     * Used after WiFi roaming when the endpoint IP hasn't changed but
     * the socket is still bound to the old network path.
     *
